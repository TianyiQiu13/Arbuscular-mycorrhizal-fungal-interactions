...
####qcmi####
edgelist_rpoB <- read.csv("rpoB_microeco_edge.csv",row.names = 1)
ig_rpoB <- graph_from_edgelist(as.matrix(edgelist_rpoB[,1:2]),directed = F)
ig_rpoB <- set_edge_attr(ig_rpoB,"weight",index=E(ig_rpoB),as.numeric(edgelist_rpoB[,3]))

otu_table_rpoB <- as.data.frame(t(vegan_otu(ps_rpoB)))

results_dl_rpoB <- assigned_process(link_table_row = edgelist_rpoB,OTUabd = otu_table_rpoB,
                                    p=0.05,data=geo,cutoff = 0,method = "dl")
results_moisture_rpoB <- assigned_process(link_table_row = edgelist_rpoB,OTUabd = otu_table_rpoB,
                                          p=0.05,data=env['Moisture'],cutoff = 0,method = "ef")
results_pH_rpoB <- assigned_process(link_table_row = edgelist_rpoB,OTUabd = otu_table_rpoB,
                                    p=0.05,data=env['pH'],cutoff = 0,method = "ef")
results_rpoBD_rpoB <- assigned_process(link_table_row = edgelist_rpoB,OTUabd = otu_table_rpoB,
                                       p=0.05,data=env['BD'],cutoff = 0,method = "ef")
results_SOC_rpoB <- assigned_process(link_table_row = edgelist_rpoB,OTUabd = otu_table_rpoB,
                                     p=0.05,data=env['SOC'],cutoff = 0,method = "ef")
results_N.P_rpoB <- assigned_process(link_table_row = edgelist_rpoB,OTUabd = otu_table_rpoB,
                                     p=0.05,data=env['N.P'],cutoff = 0,method = "ef")
results_TP_rpoB <- assigned_process(link_table_row = edgelist_rpoB,OTUabd = otu_table_rpoB,
                                    p=0.05,data=env['TP'],cutoff = 0,method = "ef")

total_link_rpoB <- row.names(edgelist_rpoB)
ef_link_rpoB <- union(as.character(row.names(results_moisture_rpoB)),
                      as.character(row.names(results_pH_rpoB)))
ef_link_rpoB <- union(ef_link_rpoB,as.character(row.names(results_rpoBD_rpoB)))
ef_link_rpoB <- union(ef_link_rpoB,as.character(row.names(results_SOC_rpoB)))
ef_link_rpoB <- union(ef_link_rpoB,as.character(row.names(results_N.P_rpoB)))
ef_link_rpoB <- union(ef_link_rpoB,as.character(row.names(results_TP_rpoB)))
dl_link_rpoB <- as.character(row.names(results_dl_rpoB))
bi_link_rpoB <- setdiff(total_link_rpoB,ef_link_rpoB)
bi_link_rpoB <- setdiff(bi_link_rpoB,dl_link_rpoB)
efedge_rpoB <- edgelist_rpoB[ef_link_rpoB,]
dledge_rpoB <- edgelist_rpoB[dl_link_rpoB,]
biedge_rpoB <- edgelist_rpoB[bi_link_rpoB,]

ig.bi_rpoB <- graph_from_edgelist(as.matrix(biedge_rpoB[,1:2]),directed = F)
ig.bi_rpoB <- set_edge_attr(ig.bi_rpoB,"weight",index=E(ig.bi_rpoB),as.numeric(biedge_rpoB[,3]))
results_rpoBi_rpoB <- qcmi(igraph = ig.bi_rpoB,OTU = otu_table_rpoB,pers.cutoff = 0)
bi_rpoB <- cbind.data.frame(results_rpoBi_rpoB[[3]],results_rpoBi_rpoB[[4]])
colnames(bi_rpoB) <- c("PNA_rpoB","PPA_rpoB")
write.csv(bi_rpoB,"rpoB_PA2.csv")
